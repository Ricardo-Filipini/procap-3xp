...
// --- PROCAP EXAM FUNCTIONS ---

export const getProcapQuestions = async (): Promise<ProcapExamQuestion[]> => {
    if (!checkSupabase()) return [];
    const data = await fetchTable('procap_exam_questions', { ordering: { column: 'question_number', options: { ascending: true } } });
    return data;
};

export const getUserExamAnswers = async (userId: string): Promise<UserExamAnswer[]> => {
    if (!checkSupabase()) return [];
     const { data, error } = await supabase!
    ...
    return data as UserExamAnswer[];
};

export const getAllUserExamAnswers = async (): Promise<UserExamAnswer[]> => {
    if (!checkSupabase()) return [];
    // Fetch all answers to calculate majority
    return fetchTable('user_exam_answers');
};

export const saveUserExamAnswer = async (userId: string, questionNumber: number, selectedAnswer: string): Promise<UserExamAnswer | null> => {
    ...
    const { data, error } = await supabase!
        .from('user_exam_answers')
        .upsert(
            { user_id: userId, question_number: questionNumber, selected_answer: selectedAnswer, updated_at: new Date().toISOString() },
...
export const deleteUserExamAnswer = async (userId: string, questionNumber: number): Promise<boolean> => {
    if (!checkSupabase()) return false;
    const { error } = await supabase!
        .from('user_exam_answers')
        .delete()
        .match({ user_id: userId, question_number: questionNumber });
